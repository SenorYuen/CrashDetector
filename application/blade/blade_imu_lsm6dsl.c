...
static void Bladelsm6dslSM_Idle(void)
{
  ...
  if( IsTimeUp(&Bladelsm6dsl_u32Timeout, U32_MEASUREMENT_RATE_MS) )
  {
    ...
    u8* au8LCDTestOut = au8Outputmessage + 6;
    u8 au8CrashMessage[] = "You have crashed!";
    static bool bEnableMoreCrashAlerts = FALSE;
    
    PWMAudioSetFrequency(BUZZER1, 500);
    LcdMessage(LINE1_START_ADDR, au8LCDTestOut);
    
    /* Best results: 12, 18 */
    if (((au8LCDTestOut[12] - 0x30) == 0) && ((au8LCDTestOut[18] - 0x30) == 0)) {
      LedOn(RED);
      LedOn(LCD_RED);
      LedOff(LCD_GREEN);
      LedOff(LCD_BLUE);
      LcdMessage(LINE2_START_ADDR, au8CrashMessage);
      if(bEnableMoreCrashAlerts) {
        PWMAudioOn(BUZZER1);
      }
    }
    else {
    ...
    if (WasButtonPressed(BUTTON1)) {
      ...
      if(bEnableMoreCrashAlerts) {
        bEnableMoreCrashAlerts =  FALSE;
        LedOff(GREEN);
      }
      else {
       bEnableMoreCrashAlerts = TRUE; 
       LedOn(GREEN);
      }
